    ...
    void* userdata)
{
    ...
    switch (event) {
        ...
        case AVAHI_BROWSER_NEW:
            ...
            // Use AVAHI_PROTO_UNSPEC to indicate both IPv4 and IPv6 should be checked
...
static void client_callback(AvahiClient *c, AvahiClientState state, AVAHI_GCC_UNUSED void * userdata) {
    ...
    switch (state) {
        case AVAHI_CLIENT_S_RUNNING:
            ...
            printf("✅ Connection to Avahi daemon established. Starting search for type **%s**...\n", SERVICE_TYPE);
            // Create a new service browser instance
            // Use AVAHI_PROTO_UNSPEC to browse all protocols (IPv4 and IPv6)
        ...
        case AVAHI_CLIENT_FAILURE:
            fprintf(stderr, "❌ Connection to Avahi server failed: %s\n", avahi_strerror(avahi_client_errno(c)));
            avahi_simple_poll_quit(simple_poll);
            break;
...
int main(AVAHI_GCC_UNUSED int argc, AVAHI_GCC_UNUSED char*argv[]) {
    ...
    // 3. Run the main loop
    printf("Starting Avahi client. (Press CTRL+C to stop)\n");
